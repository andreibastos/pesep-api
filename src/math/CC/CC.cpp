    	...
    	{	
			...
			if (tipo == 2)							//Falta bifï¿½sica sem terra
			{
				...
				if (tipo == 4)					 //Falta bifï¿½sica com terra
				{
					...
					i_f_g = sqrt((i_f_zero2+0.5*(i_f-i_f_n2))*(i_f_zero2+0.5*(i_f-i_f_n2))+(sqrt(3)*(-i_f-i_f_n2)/2)*(sqrt(3)*(-i_f-i_f_n2)/2)); //Obtenï¿½ï¿½o da corrente de falta de seguï¿½ncia positiva
					//a_f_g = (180/3.14)*atan((-sqrt(3)/2)*(i_f-i_f_n2)/(i_f_zero2-(i_f+i_f_n2)/2))+90;
					a_f_g = 90+(180/3.14)*atan((sqrt(3)*(-i_f-i_f_n2)/2)/(i_f_zero2+0.5*(i_f-i_f_n2))); //Obtenï¿½ï¿½o do ï¿½ngulo da corrente de falta de seguï¿½ncia positiva

...
param bus_g_shunt    {BUS};				# Condutância do Banco de capacitores
param bus_b_shunt   {BUS};				# Susceptância inicial do Banco de capacitores
#param bus_b_shunt_min{BUS};				# Susceptância mínima do Banco de capacitores
#param bus_b_shunt_max{BUS};				# Susceptância máxima do Banco de capacitores
param bus_area       {BUS};				#MANTER!
param bus_p_gen_min  {BUS};				# Potência ativa mínima no gerador
param bus_p_gen_max  {BUS};				# Potência ativa máxima no gerador

param bus_q_shunt  {BUS};				# TESTE!

# Dados das linhas

param branch_type    {BRANCH};			#RETIRAR
param branch_r       {BRANCH};			# Resistência 
param branch_x       {BRANCH};			# Indutância
param branch_c       {BRANCH};			# Capacitância
...
#var bus_b_shunt {i in BUS};	 # Condutância do Banco de capacitores das barras
#var bus_b_shunt {i in BUS} >= bus_b_shunt_min[i], <= bus_b_shunt_max[i];	 # Condutância do Banco de capacitores das barras
...
if(k == m) then (sum{(l,k,i) in BRANCH} (branch_b[l,k,i]*branch_tap[l,k,i]^2 + branch_c[l,k,i]/2)
...
#let bus_q_shunt[1] := 9;
#let bus_q_shunt[2] := 9;
#let bus_q_shunt[3] := 10.25;
#let bus_q_shunt[4] := 10.25;
...
for{i in BUS : bus_type[i] == 0 || bus_type[i] == 2} {	# Torna as tensões e ângulos das barras PV e PQ em 1 e 0
	let bus_voltage[i] := 1;
   let bus_angle[i] := 0;
  };
...
for{i in BUS} {											# Coloca as potências em pu
   #let bus_b_shunt[i] := bus_b_shunt[i];
...
fix {i in BUS : bus_type[i] == 3 || bus_type[i] == 2} bus_voltage[i] := bus_voltage0[i]; 	# Fixa a tensão da barra Slack
...
#let bus_angle[2] := -0.976*3.14159/180;
#let bus_voltage[3] := 0.969;
#let bus_angle[3] := -1.872*3.14159/180;
#let bus_angle[4] := 1.523*3.14159/180;


  for {i in BUS }{
	printf "%f\n", bus_voltage[i];
	printf "%f\n", bus_angle[i]*180/3.14159;
}
...
for{(l,k,m) in BRANCH} {
  ...
  let q_d[ l,k,m] :=-(branch_b[l,k,m]+branch_c[l,k,m]/2)*bus_voltage[ k]^2*branch_tap[l,k,m]^2 

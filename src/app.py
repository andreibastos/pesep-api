...
from flask import Response, Flask, jsonify, abort, request, url_for, \
...
def page_not_found(e):
    return Response(json.dumps({'message_error': 'Route not exist.  Please \
                    usage: /calcule/power_flower or /calcule/short_circuit ',
                                'code': 404, 'error': True}),
                    status=404,
...
def handle_invalid_usage(error):
    return Response(json.dumps(error.to_dict()),
                    status=200,
                    mimetype='application/json')
...
def power_flow():
    ...
    if (data):
        ...
        response = Response(json.dumps(flow), status=200,
                            mimetype='application/json')
        return response
    else:
...
def short_circuit():
    ...
    if (data):
        try:
            ...
            response = Response(json.dumps(short), status=200,
                                mimetype='application/json')
            return response
